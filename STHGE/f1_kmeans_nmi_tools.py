...
def kmeans_test(X, y, n_clusters, repeat=10):       # 输入为特征矩阵、特征矩阵、聚类簇数（如果 None，默认等于 y 中类别数量），返回nmi和ari
    nmi_list = []
    ari_list = []
    for _ in range(repeat):
        ...
        nmi_score = normalized_mutual_info_score(y, y_pred, average_method='arithmetic')    # 衡量 y_pred 和 y 的相似性
...
def svm_test(X, y, test_sizes=(0.2, 0.4, 0.6, 0.8), repeat=10):
    ...
    for test_size in test_sizes:
        ...
        for i in range(repeat):
            ...
            # 训练SVM分类器。
            svm = LinearSVC(dual=False)     # 如果 样本数n_samples < 特征数n_features，则应设置 dual=True（默认）
...
def parse_minibatch(adjlists, edge_metapath_indices_list, idx_batch, device, samples=None):
    ...
    for adjlist, indices in zip(adjlists, edge_metapath_indices_list):
        ...
        if len(edges) > 0:
            ...
            sorted_index = sorted(range(len(edges)), key=lambda i: edges[i])
            #q = list(zip(*sorted(map(lambda tup: (int(tup[1]), int(tup[0])), edges))))
            q = list(zip(*[(edges[i][1], edges[i][0]) for i in sorted_index]))
            g = dgl.graph((torch.LongTensor(q[0]).to(device), torch.LongTensor(q[1]).to(device)))
            result_indices = torch.LongTensor(result_indices[sorted_index]).to(device)
        else:
